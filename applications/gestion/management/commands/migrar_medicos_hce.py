...
class Command(BaseCommand):
    help = "Migra médicos desde SQL Server HCE.dbo.Profesionales hacia Medico"
    ...
    def handle(self, *args, **options):
        ...
        sql = f"""
            SELECT {top}
                ...
                p2.Apellido,
                p.Matricula,
                u.NombreInicioSesion AS Firma
            FROM dbo.Profesionales p
            INNER JOIN dbo.Personas p2
                ON p2.Id = p.IdPersona
            LEFT JOIN dbo.Usuarios u
                ON u.Id = p.IdUsuario
            WHERE p.ActivoActualmente = 1
              AND p.Estado = 'A'
              AND p2.Apellido NOT LIKE '%prueba%'
              AND p2.Nombres NOT LIKE '%prueba%'
            ...
            with transaction.atomic():
                while True:
                    ...
                    for fila in filas:
                        ...
                        try:
                            matricula = limpiar_texto(data.get("Matricula"))
                            ...
                            defaults = {
                                ...
                                "firma_nombre": limpiar_texto(data.get("Firma")),
                                "servicio": None,
                            }
                            ...
                            errores += 1
                            self.stdout.write(self.style.ERROR(
                                f"Error procesando matrícula {data.get('Matricula')}: {e}"
                            ))
